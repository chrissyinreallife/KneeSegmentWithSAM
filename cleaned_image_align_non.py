...
def extract_specific_value_coords_and_label(data, label):
    # Find the maximum value in the data to use as the specific value
    ...
    for slice_index in range(data.shape[2]):
        ...
        # Assuming filter_similar_coords is a function that filters out similar coordinates
        # based on a specified grid size.
        if coords.size > 0:
    ...
    if not coords_labels_list:  # Debug line to check if the list is empty
...
def combine_coords_labels(pos_labels, neg_labels):
    ...
    total_slices = 36  # Total number of slices is fixed at 40

    # Iterate through all possible slice indices, from 0 to 39
...
def save_formatted_data(formatted_list, filename):
    with open(filename, 'w') as file:
        # Iterate through the formatted_list to write each item
        for i, item in enumerate(formatted_list):
            # Check if the current item is the last in the list
            if i == len(formatted_list) - 1:
                # If it is the last item, don't add a comma at the end
                file.write(f"{item}\n")
            else:
                # If it is not the last item, add a comma at the end
                file.write(f"{item},\n")

def transform_coords(coords, image_shape):
    """
    Rotates coordinates 90 degrees clockwise and flips them along the y-axis.

    :param coords: numpy array of coordinates (N, 2) where N is the number of points.
    :param image_shape: tuple (height, width) of the image.
    :return: numpy array of transformed coordinates.
    """
...
def filter_coords_by_intensity(image, coords, labels, low_intensity_threshold=400, high_intensity_threshold=900):
    """
    Filter out coordinates based on intensity thresholds for labels 1 and 0.
    
    :param image: The slice image data.
    :param coords: Coordinates of points.
    :param labels: Labels corresponding to coords.
    :param low_intensity_threshold: Intensity threshold below which points with label 1 are removed.
    :param high_intensity_threshold: Intensity threshold above which points with label 0 are removed.
    :return: Filtered coordinates and labels.
    """
    filtered_coords = []
    filtered_labels = []
...
def parse_coords_and_labels_from_file(file_path):
    ...
    with open(file_path, 'r') as file:
        # Assuming the file structure is a series of dictionaries, one per line
        for line in file:
            # Safely evaluate the dictionary string to a Python dictionary
            slice_dict = ast.literal_eval(line.strip(",\n"))
            for key in slice_dict.keys():
                if 'coords' in key or 'labels' in key:
                    # Convert string representation of numpy arrays into actual numpy arrays
...
def majority_voting(nii_file_paths, ground_truth_mask):
    """
    Perform majority voting across multiple mask files.
    Each mask containing only two values will be converted to 0 or 1.
    :param nii_file_paths: List of paths to the .nii files containing the masks.
    :return: A 3D numpy array representing the combined mask after majority voting.
    """
    masks = []
    masks = []
...
def volumetric_overlap_error(mask1, mask2):
    """Calculate the Volumetric Overlap Error between two masks."""
...
def volume_difference(mask1, mask2):
    """Calculate the volume difference between two masks using the specified equation.
    
    Args:
        mask1 (np.ndarray): Segmentation mask.
        mask2 (np.ndarray): Reference (ground truth) mask.
    
    Returns:
        float: The calculated volume difference.
    """
    # Calculate the volumes of mask1 and mask2
    volume_s = np.sum(mask1)
    volume_r = np.sum(mask2)
    
    # Avoid division by zero
    if volume_r == 0:
        return 0
    
    # Calculate the volume difference using the given formula
    vd = 100 * np.abs(volume_s - volume_r) / volume_r
    return vd


def surface_distances(mask1, mask2):
    """Compute surface distances for AvgD and RMSD calculations."""
    eroded_mask1 = binary_erosion(mask1)
    eroded_mask2 = binary_erosion(mask2)
    surface_mask1 = np.logical_xor(mask1, eroded_mask1)
    surface_mask2 = np.logical_xor(mask2, eroded_mask2)
    distance_transform2 = ndimage.distance_transform_edt(~surface_mask2) # Use ~ for logical NOT
    distance_transform1 = ndimage.distance_transform_edt(~surface_mask1) # Use ~ for logical NOT
    surface_distance1 = distance_transform2 * surface_mask1
    surface_distance2 = distance_transform1 * surface_mask2
    return surface_distance1[surface_distance1 > 0], surface_distance2[surface_distance2 > 0]

def avg_surface_distance(surface_distances):
    """Compute the average surface distance."""
    distances1, distances2 = surface_distances
    return (np.mean(distances1) + np.mean(distances2)) / 2

def rms_surface_distance(surface_distances):
    """Compute the root mean square surface distance."""
    distances1, distances2 = surface_distances
    return np.sqrt((np.mean(distances1**2) + np.mean(distances2**2)) / 2)

def calculate_metrics(combined_mask, ground_truth_mask):
    #surface_distances_values = surface_distances(combined_mask, ground_truth_mask)
    #avgD = avg_surface_distance(surface_distances_values)
    #rmsD = rms_surface_distance(surface_distances_values)
    #vd = volume_difference(combined_mask, ground_truth_mask)
    #voe = volumetric_overlap_error(combined_mask, ground_truth_mask)
...
desired_depth = 36
REF_DIR = '/home/yaxi/ref'
NII_DIR = '/home/yaxi/test'
PROP_DIR = '/home/yaxi/prompt'
LABEL_DIR = '/home/yaxi/label'


#niftyreg fn path
reg_aladin_path = '/home/yaxi/niftyreg-git/build/reg-apps/reg_aladin'
reg_f3d_path = '/home/yaxi/niftyreg-git/build/reg-apps/reg_f3d'
reg_transform_path = '/home/yaxi/niftyreg-git/build/reg-apps/reg_transform'
reg_resample_path = '/home/yaxi/niftyreg-git/build/reg-apps/reg_resample'
...
for id in ids:
    ...
    for num in nums:
        ...
        ref_file = f'/home/yaxi/ref/000{num}.nii'
        ...
        sam_nii_file = f"{reg_save_dir}/final_res_{id}_{num}.nii"

        save_dir = f'/home/yaxi/inference'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        try:
            nii_img = nib.load(sam_nii_file)
            nii_data = nii_img.get_fdata()
            nii_data = standardize_intensity(nii_data)

        except FileNotFoundError:
            print(f"File not found: {sam_nii_file}")
            continue
        except Exception as e:
            print(f"An error occurred while processing {id}: {e}")
            continue

        sam_checkpoint = '/home/yaxi/sam_vit_h_4b8939.pth'
        model_type = "vit_h"
...
# Save the masks as a NIfTI file
        mask_file = f"{save_dir}/{id}_mask{num}.nii"
        nib.save(nii_mask_img, mask_file)  
        
        # masks resampling
        mask_mid_file = f"{save_dir}/mid_{id}_mask{num}.nii"
        if os.path.exists(prop_pos_file):
            resample_pos_command= [reg_resample_path, "-ref", aff_res_file, "-flo", mask_file, "-trans", inv_cpp_matrix, "-res", mask_mid_file]
            if not execute_command(resample_pos_command):
                print(f"Failed to resample positive prompt for {id} with reference {num}")
        
        mask_final_file = f"{save_dir}/final_{id}_mask{num}.nii"
        if os.path.exists(prop_pos_file):
            resample_pos_command= [reg_resample_path, "-ref", nii_file, "-flo", mask_mid_file, "-trans", inv_aff_matrix, "-res", mask_final_file]
            if not execute_command(resample_pos_command):
                print(f"Failed to resample positive prompt for {id} with reference {num}")
        

    five_mask_paths = [f'{save_dir}/final_{id}_mask0.nii', f'{save_dir}/final_{id}_mask1.nii', f'{save_dir}/final_{id}_mask2.nii', f'{save_dir}/final_{id}_mask3.nii', f'{save_dir}/final_{id}_mask4.nii']
    ...
    final_filename = os.path.join(save_dir, f'final_combined_{id}_mask.nii')
